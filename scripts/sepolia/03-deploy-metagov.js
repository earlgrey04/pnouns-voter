// MetaGov を Sepolia にデプロイ(Nouns 公式 Sepolia、pNouns 複製、margin=MARGIN(既定 5 ブロック: 投票期間 25 ブロックのため))
...
async function main() {
  ...
  dep.metagov = await c.getAddress();
  dep.metagovDeployBlock = (await c.deploymentTransaction().wait()).blockNumber;
  dep.marginBlocks = Number(margin);
  saveDeployments(dep);
  const tx = await c.setLiveMode(true);
  await tx.wait();
  console.log("MetaGov:", dep.metagov, "liveMode=true margin=", String(margin));
}
