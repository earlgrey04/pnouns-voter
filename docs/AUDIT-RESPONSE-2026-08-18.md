> **注記(2026-08-24)**: 公開に伴う個人情報(コミット author のメールアドレス等)の除去のため、
> 同日に git 履歴を書き換えました。本書および監査生ログ中で参照しているコミット ID は
> 書き換え前のものであり、現在のリポジトリのコミット ID とは一致しません(内容の対応は
> コミットメッセージで辿れます)。
...
## 第27回監査 (2026-08-21, Codex) — コード面クローズ + E2E 証明範囲の確定

対象: 5e08d48。生ログ: `docs/audit-27-codex-raw.md`
**判定: コード面クローズ可。重大・中の残件なし。** 第26回の 3 主張(deployBlock 検証・
placeholder 停止・getLogs assert)はすべて成立。

低(非ブロッキング): 未来ブロック/巨大値の deployBlock は形式上受理される(誤設定時は
古い提案の逆引きで unresolved 停止=安全側)。将来 /api/config に deployBlock を出して
現在ブロック以下か照合するのは改善案。クローズ阻害せず。
注意: RUNBOOK の 7200 は「締切マージン」で登録猶予(10)とは別 → 用語区別を明記。

### E2E 証明範囲(Codex 確定・本番 live 化条件)

| 項目 | 検証先 |
|---|---|
| 多数票の複数 tick 分割排出 | **Sepolia 追加実測(live化条件)**: 11〜21 票で複数 tick・receipt 後再走査・取りこぼしなし |
| 直近 20 件外のイベント逆引き | **Sepolia 実 RPC で追加検証必須**: 古い登録済み ID を resolveMappings に与える統合確認 |
| 長時間障害と復旧 | 本番見学モード(cron 停止→復旧・二重計上なし) |
| 本番 7200 マージンの実運用(4 日間) | 本番見学モード |
| 締切間際の backlog/graceBad 停止 | 本番見学モード(停止訓練) |

**本番移行の道筋(確定)**: ①コア・登録経路のコードはクローズ(達成) → ②Sepolia で
「複数 tick 排出」と「直近20件外逆引き」を追加実測 → ③mainnet 見学モードで 4 日間・
