...
- `MARGIN=7200` (約 24 時間 — 締切マージン。集計の締切 = Nouns 投票終了の 24 時間前)
- **用語の区別**: 登録猶予(REG_DELAY=10 ブロック・約 2 分)と締切マージン(MARGIN=7200 ブロック・約 24 時間)は別物。前者は誤登録の確認間隔、後者は投票終了前に集計を締め切る余裕
